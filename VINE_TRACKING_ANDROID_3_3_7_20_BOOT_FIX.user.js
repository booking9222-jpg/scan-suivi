...
(() => {
  ...
  function migrateConservativeConfig(stored) {
    ...
    if (src.dedicatedDeviceMode) {
      ...
      if (!Number.isFinite(Number(src.bufferShipmentLimit)) || Number(src.bufferShipmentLimit) <= 16) src.bufferShipmentLimit = 32;
      if (!Number.isFinite(Number(src.bufferItemLimit)) || Number(src.bufferItemLimit) <= 48) src.bufferItemLimit = 96;
    ...
    src.bufferShipmentLimit = Math.min(64, Math.max(8, positive(src.bufferShipmentLimit, src.dedicatedDeviceMode ? 32 : 16)));
    src.bufferItemLimit = Math.min(192, Math.max(24, positive(src.bufferItemLimit, src.dedicatedDeviceMode ? 96 : 48)));
...
${txt}`;
  ...
  async function flushBuffer(state, force = false) {
    ...
    const b={ shipments:[...state.buffer.shipments], items:[...state.buffer.items] };
    state.lastMessage = `Google : envoi de ${b.shipments.length} colis / ${b.items.length} article(s)…`;
    setScanOperation(state, 'google-flush', { shipments:b.shipments.length, items:b.items.length });
    await apiPost('upsertBatch',b);
    // Ne vider que les éléments réellement envoyés. Si de nouvelles données ont été
    // ajoutées au buffer pendant une reprise, elles restent disponibles.
    ...
    state.lastMessage = `Google OK : ${b.shipments.length} colis / ${b.items.length} article(s) envoyés`;
  ...
  async function finishScan(state) {
    ...
    try {
      ...
      state.active = false;
      state.paused = true;
      state.lastMessage = `Pause : impossible d’envoyer les dernières données (${e.message}). Appuie sur Reprendre.`;
      saveState(state);
      recordScanSummary(state, 'ERREUR', state.lastMessage);
      toast(state.lastMessage, 'error');
      return;
