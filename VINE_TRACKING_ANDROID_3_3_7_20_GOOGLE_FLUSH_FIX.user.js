...
(() => {
  ...
  const VERSION = '3.3.7';
  const BUILD_VERSION = '3.3.7.20';
  const HOTFIX_GOOGLE_FLUSH = 'microbatch-8x24-backoff';
  ...
  const CONFIG_POLICY_REV = 'V33720_QUEUE_FLEET_1';
  ...
  const DEFAULT_CONFIG = {
    ...
    // Téléphones dédiés Vine : la RAM sert aux files/cache, mais les envois Apps Script restent en micro-lots.
    // Des lots 32/96 ont été observés bloqués sur google-flush ; 8/24 borne le temps serveur.
  ...
  function migrateConservativeConfig(stored) {
    ...
    // Hotfix google-flush : garder les gros caches/queues locaux, mais ne jamais
    // attendre d'avoir 32 colis avant de parler à Apps Script. Les anciennes
    // valeurs 32/96 sont rabattues automatiquement à 8/24.
    if (src.dedicatedDeviceMode) {
      ...
      src.bufferShipmentLimit = Math.min(8, Math.max(1, positive(src.bufferShipmentLimit, 8)));
      src.bufferItemLimit = Math.min(24, Math.max(1, positive(src.bufferItemLimit, 24)));
      src.priorityFlushShipmentLimit = Math.min(6, Math.max(2, positive(src.priorityFlushShipmentLimit, 4)));
      src.priorityFlushItemLimit = Math.min(12, Math.max(4, positive(src.priorityFlushItemLimit, 8)));
      if (!Number.isFinite(Number(src.priorityFlushMaxAgeMs)) || Number(src.priorityFlushMaxAgeMs) > 90000) src.priorityFlushMaxAgeMs = 60000;
    }
    ...
    src.priorityFlushShipmentLimit = Math.min(6, Math.max(2, positive(src.priorityFlushShipmentLimit, src.dedicatedDeviceMode ? 4 : 3)));
    src.priorityFlushItemLimit = Math.min(12, Math.max(4, positive(src.priorityFlushItemLimit, src.dedicatedDeviceMode ? 8 : 6)));
    src.priorityFlushMaxAgeMs = Math.min(2 * 60 * 1000, Math.max(30 * 1000, positive(src.priorityFlushMaxAgeMs, 60 * 1000)));
...
${txt}`;
  ...
  async function apiPost(action, data = {}) {
    const c = getConfig();
    // Un timeout upsertBatch peut laisser l'exécution Apps Script continuer côté serveur.
    // Ne pas relancer immédiatement le même write : cela crée du lock contention.
    const normalAttempts = action === 'upsertBatch' ? 1 : Math.max(1, Math.min(2, Number(c.apiRetries || 2)));
    const busyAttempts = action === 'upsertBatch' ? 3 : (action === 'finalizeSync' ? 4 : normalAttempts);
    let lastErr;
  ...
  function googleFlushChunk_(state) {
    const c=getConfig();
    const shipMax=Math.max(1,Math.min(8,Number(c.bufferShipmentLimit||8)));
    const itemMax=Math.max(1,Math.min(24,Number(c.bufferItemLimit||24)));
    const shipments=(state.buffer?.shipments||[]).slice(0,shipMax);
    const shipKeys=new Set(shipments.map(x=>String(x?.shipmentKey||'')).filter(Boolean));
    let items=[];
    if(shipKeys.size) items=(state.buffer?.items||[]).filter(x=>shipKeys.has(String(x?.shipmentKey||''))).slice(0,itemMax);
    if(!items.length) items=(state.buffer?.items||[]).slice(0,itemMax);
    return {shipments,items};
  }

  async function flushBuffer(state, force = false, options = {}) {
    state.buffer ||= {shipments:[],items:[]};
    ...
    const c=getConfig(), shipmentLimit=Math.max(1,Number(c.bufferShipmentLimit||8)), itemLimit=Math.max(1,Number(c.bufferItemLimit||24));
    const strict=options?.strict===true, drainAll=options?.drainAll===true;
    const totalPending=()=>Number(state.buffer?.shipments?.length||0)+Number(state.buffer?.items?.length||0);
    if(!force&&state.buffer.shipments.length<shipmentLimit&&state.buffer.items.length<itemLimit)return false;
    if(!totalPending())return false;

    const backoffUntil=Number(state.googleBackoffUntil||0);
    if(!strict && backoffUntil>Date.now()){
      state.lastMessage=`Google différé · ${state.buffer.shipments.length} colis / ${state.buffer.items.length} article(s) conservés localement`;
      saveState(state); return false;
    }

    let sentAny=false, chunkNo=0;
    while(totalPending()){
      const b=googleFlushChunk_(state);
      if(!b.shipments.length&&!b.items.length)break;
      chunkNo++;
      state.lastMessage=`Google : micro-lot ${chunkNo}${drainAll?'':'/1'} · ${b.shipments.length} colis / ${b.items.length} article(s)…`;
      setScanOperation(state,'google-flush',{shipments:b.shipments.length,items:b.items.length,pendingShipments:state.buffer.shipments.length,pendingItems:state.buffer.items.length,chunk:chunkNo});
      try{
        await apiPost('upsertBatch',b);
      }catch(e){
        const failures=Number(state.googleSyncFailures||0)+1;
        state.googleSyncFailures=failures;
        const delay=Math.min(10*60*1000,Math.max(60*1000,60*1000*Math.pow(2,Math.min(3,failures-1))));
        state.googleBackoffUntil=Date.now()+delay;
        state.lastGoogleError={at:nowIso(),message:String(e?.message||e),code:String(e?.code||''),failures,retryAt:new Date(state.googleBackoffUntil).toISOString()};
        clearScanOperation(state);
        state.lastMessage=`Google temporairement indisponible : données conservées (${state.buffer.shipments.length} colis / ${state.buffer.items.length} article(s)). Scan Amazon continue ; nouvel essai dans ~${Math.round(delay/60000)} min.`;
        saveState(state);
        if(strict)throw e;
        return sentAny;
      }

      const sentShipKeys=new Set(b.shipments.map(x=>x.shipmentKey));
      const sentItemKeys=new Set(b.items.map(x=>x.itemKey));
      state.buffer.shipments=(state.buffer.shipments||[]).filter(x=>!sentShipKeys.has(x.shipmentKey));
      state.buffer.items=(state.buffer.items||[]).filter(x=>!sentItemKeys.has(x.itemKey));
      state.lastBufferFlushAt=nowIso();state.googleSyncFailures=0;state.googleBackoffUntil=0;delete state.lastGoogleError;
      clearScanOperation(state);sentAny=true;
      state.lastMessage=`Google OK : micro-lot ${b.shipments.length} colis / ${b.items.length} article(s) · reste ${state.buffer.shipments.length}/${state.buffer.items.length}`;
      saveState(state);
      if(!drainAll)break;
    }
    return sentAny;
  ...
  async function resumePausedScan() {
    ...
    // Une pause causée par un ancien google-flush ne doit pas réarmer le watchdog
    // avec l'horodatage figé. Reprendre force aussi un essai Google immédiat.
    if (String(s.operation?.kind || '').startsWith('google-')) delete s.operation;
    s.googleBackoffUntil = 0;
    s.active = true;
    s.paused = false;
    s.retryCounts = {};
  ...
  async function finishScan(state) {
    ...
    state.lastMessage = 'Envoi final… compteurs provisoires jusqu’au recalcul Google'; saveState(state);
    try {
      await flushBuffer(state, true, { strict:true, drainAll:true });
    } catch (e) {
      state.stats.errors++;
      state.active = false;
      state.paused = true;
      state.lastMessage = `Pause finale : données Amazon conservées localement, mais Google reste indisponible (${e.message}). Appuie sur Reprendre pour retenter uniquement la synchronisation.`;
      saveState(state);
      recordScanSummary(state, 'ERREUR', state.lastMessage);
      toast(state.lastMessage, 'error');
      return;
    ...
    try {
      ...
      state.active = false;
      state.paused = true;
      state.lastMessage = `Données envoyées, mais finalisation impossible (${e.message}). Appuie sur Reprendre.`;
      saveState(state);
      recordScanSummary(state, 'ERREUR', state.lastMessage);
      toast(state.lastMessage, 'error');
      return;
  ...
  async function processVineOrdersIndex(state) {
    ...
    for (const candidate of candidates) {
      ...
      const extensionReuse = state.mode==='range' && canReuseCandidateFromExtension(state,cached,orderDate,serverHasOrder,rangePending) && state.forceFullScan!==true;
      if (extensionReuse) { state.stats.rangeReused=(state.stats.rangeReused||0)+1; continue; }
  ...
  async function processHistory(state) {
    ...
    if (state.mode !== 'search') {
      ...
      serverDates = await fetchKnownOrderDates(candidates.map(x => x.orderId));
      clearScanOperation(state);
    }
    ...
    for (const candidate of candidates) {
      ...
      const lastCheckedMs = Date.parse(cached?.lastChecked || 0) || 0;
      const sinceChecked = lastCheckedMs ? Math.max(0, Date.now() - lastCheckedMs) : Infinity;
      const serverHasOrder = !!validIsoOrderDate(serverDates[candidate.orderId]);
      // Si Google a été vidé/recréé mais que le cache local connaît encore la commande,
      // on doit réhydrater Google une fois : le mode faible trafic ne doit pas rendre la
      // restauration impossible.
      const serverNeedsRehydrate = state.mode === 'range' && !!cached && !serverHasOrder;
      const incompleteNeedsRefresh = !!cached && cached.trackingComplete !== true && sinceChecked >= Math.max(30 * 60000, Number(c.incompleteRecheckMs || 7200000));
      const completedOpenNeedsRefresh = !!cached && cached.trackingComplete === true && cached.delivered !== true && cached.terminalNoTracking !== true && sinceChecked >= Math.max(2 * 3600000, Number(c.completeOpenRecheckMs || 28800000));
      // Un colis déjà livré + complet + présent dans Google est final pour le scan de
      // routine : inutile de rouvrir Amazon à chaque passage 7 jours.
      const mustRecheck = rangePending || !cached || serverNeedsRehydrate || incompleteNeedsRefresh || completedOpenNeedsRefresh;
      if (!mustRecheck && state.mode === 'range') continue;
  ...
  function buildDiagnosticText() {
    ...
    return [
      ...
      `Google flush: backoff=${getState()?.googleBackoffUntil && Number(getState().googleBackoffUntil)>Date.now() ? Math.ceil((Number(getState().googleBackoffUntil)-Date.now())/1000)+'s' : 'non'} · échecs=${getState()?.googleSyncFailures||0} · dernier=${JSON.stringify(getState()?.lastGoogleError||null)}`,
  ...
  function updatePanel(state) {
    ...
    host.querySelector('[data-role="msg"]').textContent = state?.lastMessage || 'Prêt';
  }
