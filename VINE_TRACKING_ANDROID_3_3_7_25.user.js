...
(() => {
  ...
  const VERSION = '3.3.7';
  const BUILD_VERSION = '3.3.7.25';
  // Build 24 changed client navigation only; protocol and server schema remain compatible.
  const SERVER_BUILDS = new Set(['3.3.7.23', '3.3.7.24']);
...
Commandes ${summaryOrders} · Colis ${st.shipments||0} · Suivis ${st.trackingFound||0} · Attente ${st.pending||0} · Livrés ${st.delivered||0} · Dates inconnues ${st.rangeUnknown||0} · À vérifier ${st.uncertain||0} · Erreurs ${st.errors||0}`;
  ...
  async function sendFleetHeartbeat(force=false) {
    ...
    const heartbeatKey='VT_FLEET_LAST_SENT_V1_'+String(cfg.account||'');
    const lastSent=Math.max(fleetLastSentAt,Number(GM_getValue(heartbeatKey,0)||0));
    if(Date.now()-lastSent<interval)return false;
    GM_setValue(heartbeatKey,Date.now());
...
${txt}`;
  ...
  async function apiPostOnce(action, data = {}) {
    ...
    try {
      ...
      if (action === 'upsertBatch' || action === 'finalizeSync' || !mayFallback) throw firstErr;
  ...
  async function apiPost(action, data = {}) {
    if (action === 'upsertBatch' || action === 'finalizeSync') return apiPostOnce(action, data);
    const c = getConfig();
  ...
  function googleSendSpacingMs(account, initial = false) {
    let hash=0;
    for(const ch of String(account||'')) hash=(Math.imul(hash,31)+ch.charCodeAt(0))>>>0;
    return (initial ? 2000 : 15000) + (hash%17)*1000 + Math.floor(Math.random()*13000);
  }
  function googleBackoffMs(failures) {
    const base=Math.min(480000,60000*Math.pow(2,Math.min(3,Math.max(0,failures-1))));
    // Randomize on every failure; bounded 1..10 minutes, including at the cap.
    return Math.min(600000,base+Math.floor(Math.random()*Math.min(120000,base/2)));
  }
  function deferGoogleSync(state, error) {
    const sync=state.googleSync ||= {};
    sync.failures=Number(sync.failures||0)+1;
    const delay=googleBackoffMs(sync.failures);
    sync.nextAttemptAt=Date.now()+delay;
    sync.lastError=String(error?.message||error||'Google indisponible');
    state.lastMessage=`Google temporairement indisponible — données conservées localement — ${state.phase==='done'?'collecte Amazon terminée':'scan Amazon continue'}. Réessai dans ${Math.ceil(delay/60000)} min (${sync.lastError}).`;
    delete state.operation;
    saveState(state);
  }
  ...
  async function flushBuffer(state, force = false) {
    ...
    if(!state.googleSync?.staggerInitialized) {
      const initialSync=state.googleSync ||= {};
      initialSync.staggerInitialized=true;
      initialSync.nextAttemptAt=Date.now()+googleSendSpacingMs(scanAccount(state),true);
      state.lastMessage='Envoi Google décalé pour répartir les téléphones — données conservées localement.';
      saveState(state);
      return false;
    }
    ...
    try {
      const result=await apiPost('upsertBatch',b);
      if(result?.performance) sync.serverPerformance=result.performance;
      acknowledgeMicroBatch(state.buffer,b);
      sync.failures=0; sync.nextAttemptAt=Date.now()+googleSendSpacingMs(scanAccount(state)); sync.lastError='';
  ...
  async function configureRelay() {
    ...
    try {
      ...
      if (String(r.version || '') !== VERSION || !SERVER_BUILDS.has(String(r.build || ''))) throw new Error(`Serveur derrière le relais incompatible : ${r.build || r.version || '?'}`);
  ...
  async function ensureServerCompatible() {
    try {
      ...
      if (!SERVER_BUILDS.has(String(r.build || ''))) {
  ...
  async function finishScan(state) {
    ...
    if(Number(state.googleSync?.nextAttemptAt||0)>Date.now()) {
      scheduleFinalGoogleRetry(state,state.googleSync.nextAttemptAt-Date.now()); return;
    }
  ...
  async function resumeScanInner() {
    ...
    try {
      if (state.phase === 'history') return await processHistory(state);
      if (state.phase === 'details') return await processDetails(state);
      if (state.phase === 'tracking') return await processTracking(state);
      return await finishScan(state);
    } catch (e) {
      clearScanOperation(state);
      ...
      if (['details','tracking'].includes(state.phase) && quarantineCurrent(state, e.message)) {
        ...
        saveState(state);
        try { void apiPost('clientLog', { message: state.lastMessage, details: { phase: state.phase, url: location.href, quarantine:true, finalPass } }).catch(() => {}); } catch (_) {}
  ...
  async function processTracking(state) {
    ...
    // V24 : ne jamais analyser le DOM d'une autre commande avec les ASIN du contexte.
    const actualOrderId = parseOrderId(location.href);
    if (actualOrderId && expectedOrderId && actualOrderId !== expectedOrderId) {
      const target = String(q.url || '');
      state.trackingNavigationRecovery ||= {};
      const attempts = Number(state.trackingNavigationRecovery[provisionalKey] || 0);
      if (attempts < 1 && isAmazonTrackingUrl(target) && parseOrderId(target) === expectedOrderId) {
        state.trackingNavigationRecovery[provisionalKey] = attempts + 1;
        clearScanOperation(state);
        state.lastMessage = `Page d'une autre commande (${actualOrderId}) : retour au suivi attendu ${expectedOrderId}.`;
        saveState(state);
        navigateTo(target);
        return;
      }
      throw new Error(`Contexte de suivi incohérent : attendu ${expectedOrderId}, page ${actualOrderId}. Aucune donnée de cette page associée au colis.`);
    }
  ...
  function buildDiagnosticText() {
    ...
    return [
      ...
      `Durées serveur dernier lot: ${JSON.stringify(getState()?.googleSync?.serverPerformance||{})}`,
